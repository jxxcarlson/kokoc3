...
import Model exposing (ImageAccessibility(..), ImageMode(..), Model, Msg(..))
...
view model =
    Element.row [ width fill, height (px (model.windowHeight - 70)), Background.color Widget.white, centerX ]
        [ imageLeftColumn 2 model
        , imageCenterColumn model.windowHeight 7 model
        , imageRightColumn 2 model
        ]
...
viewImageToUpload_ model =
    Html.div
        ...
        , Html.input
            [ type_ "file"
            , on "change" (decodeNodeFile ReadImage)
            , value ""
    ...
    case maybeData of
        ...
        Just data ->
            case imageType_ of
                ...
                "image/jpeg" ->
                    Html.p [] [ Maybe.map displayImage maybeData |> Maybe.withDefault (Html.text "") ]
                ...
                _ ->
                    Html.pre [] [ Html.text "Unrecognized media format" ]
...
imageType model =
    let
        imageString =
            model.maybeImageString |> Maybe.withDefault "- no image -"

        i =
            String.indexes ";" imageString |> List.head |> Maybe.withDefault 0

        imageInfo =
            String.slice 0 i imageString
    in
